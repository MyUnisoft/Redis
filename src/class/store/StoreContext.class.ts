...
import { SessionStore, SessionStoreOptions } from "./SessionStore.class";
...
* });
*/
export class StoreContext<T extends Store = Store> extends SessionStore<T> {
  protected authenticationField: keyof T | null;
  protected cookiesOptions: CookieSerializeOptions;

  constructor(options?: Partial<StoreContextOptions<T>>, redis?: Redis) {
    super(options, redis);

    this.authenticationField = options?.authentificationField ?? null;
    this.cookiesOptions = options?.setCookiesOptions ?? Object.assign({}, kDefaultCookiesOptions);
  }
  ...
  */
  async initSession(id: string, ctx: FrameworkContext, payload: T): Promise<string> {
    ...
    await this.setValue(payload, id);
...
export interface StoreContextOptions<T extends Store> extends SessionStoreOptions<T> {
